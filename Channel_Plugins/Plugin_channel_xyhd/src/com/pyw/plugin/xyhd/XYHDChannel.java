...
import com.mchsdk.open.SubmitGameInfoCallback;
...
/**
 * game_key：48f5b913 渠道號：66 × 渠道名稱：安卓-星逸互动 注意：需要將dlwxcontrol放入到assets下
 * 
 * @author zhj
 *
 */
public class XYHDChannel extends PYWPlugin {
	...
	public void init(final Context context, final HashMap params, executeCallback callback) {
		...
		MCApiFactory.getMCApi().setParams("0", // 渠道id
				...
				"27", // 游戏id(编号)
				"决战传奇(安卓版)", // 游戏名称
				"59C8660A6A47194ED"// 游戏appid
				, "LCQnMilSVl9RXFBWXgAGBAcM", // 访问秘钥：
				// (SDK访问服务器时的加密key)
				"http://101.201.117.123:8999/sdk.php/");// 联运SDK服务器地址
		// MCApiFactory.getMCApi().setParams("0", // 渠道id
		// "自然注册", // 渠道名称
		// "8", // 游戏id(编号)
		// "花丛使者(安卓版)", // 游戏名称
		// "68CC794C419315FC2"// 游戏appid
		// , "Dw0SDyEsLSUtJSktKi4=", // 访问秘钥： (SDK访问服务器时的加密key)
		// "http://101.201.117.123:8999/sdk.php/");
	...
	public void getRoleMessage(final Context context, HashMap params, final executeCallback callback) {
		...
		roleID = TextUtils.isEmpty(params.get("roleName") + "") ? "" : params.get("roleName") + "";
		String t = TextUtils.isEmpty(params.get("serverAreaName") + "") ? "" : params.get("serverAreaName") + "";
		MCApiFactory.getMCApi().submitGameInfo(serverId, t, roleID,
				TextUtils.isEmpty(params.get("roleName") + "") ? "" : params.get("roleName") + "",
				TextUtils.isEmpty(params.get("roleLevel") + "") ? "" : params.get("roleLevel") + "", roleID, roleID,
				roleID, "工会", getcall());
	}

	private SubmitGameInfoCallback c;

	private SubmitGameInfoCallback getcall() {
		if (c == null) {
			c = new SubmitGameInfoCallback() {
				@Override
	...
	public void pay(final Context context, HashMap params, final executeCallback callback) {
		...
		order.setGameServerId(TextUtils.isEmpty(params.get("serverArea") + "") ? "" : params.get("serverArea") + "");
		order.setGameServerName(
				TextUtils.isEmpty(params.get("serverAreaName") + "") ? "" : params.get("serverAreaName") + "");
		order.setRoleId(TextUtils.isEmpty(params.get("roleid") + "") ? "1" : params.get("roleid") + "");
		order.setRoleName(TextUtils.isEmpty(params.get("roleName") + "") ? "" : params.get("roleName") + "");
